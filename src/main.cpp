// // TEST 01 - Motor Control with micro-ROS on ESP32
// // This code is designed to control motors using an ESP32 with micro-ROS.
// // It sets up motor control pins, initializes them, and provides a basic loop to control the motors.
// #include <Arduino.h>
...
//   for(int i = 4; i < 5; i++)   {
//     digitalWrite(inMotorPins[i], LOW);
//     digitalWrite(inMotorPins2[i], HIGH);
//   }  

//   for(int i = 4; i < 5; i++) {
//     analogWrite(enMotorPins[i], 155);
//     delay(8000); // Wait before the next loop iteration
...
// TEST 02 - Motor Control with micro-ROS on ESP32
#include <Arduino.h>
...
// At the top of your file, after your pin‑definitions, add:

// Threshold under which we consider a speed “unchanged”
static const int SPEED_THRESHOLD = 20;

// Keep track of the last command we actually sent to each motor:
int last_speeds[6]     = { 0, 0, 0, 0, 0, 0 };
int last_directions[6] = { 0, 0, 0, 0, 0, 0 };

// Then, update your motor_command_callback (and likewise control_callback) like this:
void motor_command_callback(const void * msgin) {
  const std_msgs__msg__Int32MultiArray * m = 
    (const std_msgs__msg__Int32MultiArray *)msgin;
  if (m->data.size < 3) return;

  int index     = m->data.data[0];
  int direction = m->data.data[1];
  int speed     = m->data.data[2];

  // Check bounds
  if (index < 0 || index >= 6) return;

  // If both direction unchanged **and** speed within ±20 of last speed, skip:
  if (direction == last_directions[index] &&
      abs(speed - last_speeds[index]) <= SPEED_THRESHOLD)
  {
    // no-op
    return;
  }

  // Otherwise update the “last” arrays and actually drive the motor:
  last_directions[index] = direction;
  last_speeds[index]     = speed;
  control_motor(index, direction, speed);
}

// Do exactly the same in your control_callback:
void control_callback(const void * msgin) {
  const std_msgs__msg__Int32MultiArray * m = 
    (const std_msgs__msg__Int32MultiArray *)msgin;
  if (m->data.size < 3) return;
